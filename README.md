...
## サークル機関誌 管理アプリ（`/magazine` 以下）

このリポジトリには、上記のクレジットカードサイトとは無関係な副次アプリとして、
サークルの機関誌をオンラインで管理する画面一式が `/magazine` 以下に同居しています。
バックエンドは **Firebase**（Authentication / Firestore / Storage）を使い、
上記の Supabase 構成とは完全に独立しています。同じ Next.js アプリの一部として
`/magazine` 配下のみ Firebase を使う構成です。

### できること

- Google サインインのみでログイン（`/magazine/login`）。
- 年度ごとの号の一覧（`/magazine`）。
- 号ごとの冊子ビューア（`/magazine/issues/[issueId]`）・原稿投稿（`.../submit`）・名簿（`.../roster`）。
- 管理者専用の号横断管理画面（`/magazine/admin`）と、号ごとの管理画面
  （`.../issues/[issueId]/admin`。投稿ファイルのダウンロード、冊子セクションの追加・並び替え・削除、名簿の追加・編集）。

### セットアップ

1. [Firebase コンソール](https://console.firebase.google.com/) で新しいプロジェクトを作成する。
2. **Authentication** で「Google」サインイン方法を有効化する。
3. **Firestore Database** と **Storage** を（無料枠の）本番モードで作成する。
4. プロジェクトの設定 → 全般 → 「マイアプリ」からウェブアプリを追加し、表示される設定値を
   `.env.local` に設定する（`.env.local.example` の `NEXT_PUBLIC_FIREBASE_*` の項目）。
5. Firestore に管理者判定用のドキュメントを1件作成する:
   - コレクション `config` / ドキュメントID `adminEmails`
   - フィールド `adminEmails`（配列, string） に管理者のGoogleアカウントのメールアドレスを追加
6. セキュリティルールをデプロイする（[Firebase CLI](https://firebase.google.com/docs/cli) を利用）:
...
7. 管理者としてログイン後、`/magazine/admin` から新年度号を作成し、`.../issues/[issueId]/admin` の
   名簿欄からメンバー（名前・Googleアカウント）を登録する。

### データとルール

- Firestore のコレクション構成、権限の考え方は本アプリの仕様どおり
  `issues/{issueId}` → `submissions` / `booklet` / `roster` のサブコレクション構成です。
- `firestore.rules` / `storage.rules` が、管理者以外は他メンバーの投稿生ファイルを
  ダウンロードできないこと・自分の名簿欄以外を編集できないことをサーバー側で強制します。
- Firebase の設定値はすべて環境変数経由で読み込み、コードには直書きしていません。
